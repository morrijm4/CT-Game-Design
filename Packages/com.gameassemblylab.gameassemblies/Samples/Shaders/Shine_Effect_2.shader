Shader "Custom/2D Sprite Shine Effect 2"
{
    Properties
    {
        ...
        _ShineOffset ("Shine Offset", Range(-3, 3)) = 0
        _ShineRange ("Shine Range", Range(1, 5)) = 2
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // Create a moving value that cycles over a wider range
                float movement = frac(_Time.y * _ShineSpeed);
                movement = movement * _ShineRange - (_ShineRange / 2); // Convert to -range/2 to range/2
                
                // Calculate the position for the shine effect (45-degree angle)
                float shinePos = (IN.texcoord.x + IN.texcoord.y) - movement + _ShineOffset;
                
                // Create a smooth shine effect with wider range
